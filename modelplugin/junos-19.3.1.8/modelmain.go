...
// A plugin for the YGOT model of Junos-19.3.1.8.
package main

import (
	"fmt"
	"github.com/onosproject/config-models/modelplugin/junos-19.3.1.8/junos_19_3_1_8"
...
func (m modelplugin) UnmarshalConfigValues(jsonTree []byte) (*ygot.ValidatedGoStruct, error) {
	device := &junos_19_3_1_8.Device{}
	vgs := ygot.ValidatedGoStruct(device)

	if err := junos_19_3_1_8.Unmarshal([]byte(jsonTree), device); err != nil {
		return nil, err
	}
...
func (m modelplugin) Validate(ygotModel *ygot.ValidatedGoStruct, opts ...ygot.ValidationOption) error {
	...
	device, ok := deviceDeref.(*junos_19_3_1_8.Device)
	if !ok {
		return fmt.Errorf("unable to convert model in to junos_19_3_1_8")
	}
	return device.Validate()
}

func (m modelplugin) Schema() (map[string]*yang.Entry, error) {
	return junos_19_3_1_8.UnzipSchema()
}
